...
def motion2video_3d_two(motion, save_path, fps=25, keep_imgs=False, view=[90, 180]):
    """
    motion:
        (17, 3, N)         -> single person
        (P, 17, 3, N)      -> multi person
    """
    videowriter = imageio.get_writer(save_path, fps=fps)

    # 人数判定
    if motion.ndim == 3:
        motion = motion[np.newaxis, ...]  # (1,17,3,N)

    P, J, C, N = motion.shape
    assert J == 17 and C == 3
    ...
    for f in tqdm(range(N)):
        ...
        for p in range(P):
            ...
            color_mid, color_left, color_right = base_colors[p % len(base_colors)]
